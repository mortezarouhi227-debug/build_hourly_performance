...
def build_hourly_performance():
    ...
    print("=" * 70)
    print("🔍 HOURLY PERFORMANCE - DIAGNOSTIC")
    print("=" * 70)
    print(f"📅 B1 (Date): {b1}")
    print(f"🕐 F1 (Hour): {f1} -> selected_hour: {selected_hour}")
    print(f"👤 J1 (Shift): {j1} -> selected_shift: {selected_shift}")
    print("=" * 70)
    ...
    # ====== دیباگ: اطلاعات All_Data ======
    print("\n📊 All_Data ANALYSIS:")
    print("-" * 70)
    print(f"Total rows in All_Data: {len(rows)}")
    ...
    print(f"Task types: {sorted(task_counts.keys())}")
    print(f"FBM rows in All_Data: {task_counts.get('FBM', 0)}")
    
    if task_counts.get('FBM', 0) > 0:
        print("\n📋 Sample FBM rows (first 3):")
        fbm_samples = [r for r in rows if str(r[colTaskType]).strip() == "FBM"][:3]
        for i, r in enumerate(fbm_samples, 1):
            print(f"  {i}. Name: {r[colFullName]}, Date: {r[colDate]}, Hour: {r[colHour]}, Qty: {r[colQuantity]}")
    print("-" * 70)
    ...
    for r in rows:
        ...
        if math.isnan(dms): continue
        
        is_fbm = str(r[colTaskType]).strip() == "FBM"
        if is_fbm:
            fbm_before += 1
        
        if not (start_ms <= dms <= end_ms): continue
    ...
    print(f"Total rows after filters: {len(filtered)}")
    print(f"FBM rows before filters: {fbm_before}")
    print(f"FBM rows after filters: {fbm_after}")
    
    if fbm_before > 0 and fbm_after == 0:
        print("\n⚠️ ALL FBM ROWS WERE FILTERED OUT!")
        print("   Check: date range, hour filter, shift filter")
    print("-" * 70)
    ...
    print("-" * 70)
    for t in TASK_TYPES:
        print(f"  {t}: {len(rows_by_task[t])} rows")
    print("-" * 70)
